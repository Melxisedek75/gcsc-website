import { DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_ARCHIVE_HANDOFF } from './localReplayApprovalDecisionExternalOwnerResponseDecisionEvidenceArchiveHandoff.mjs';
...
export const REQUIRED_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_ARCHIVE_HANDOFF_CLOSEOUT_FIELDS = Object.freeze([
  ...
  'digest_id',
  'module_order',
  'decision_evidence_archive_handoff_closeout_status',
  'source_decision_evidence_archive_handoff_status',
  'closed_owner_review_handoff_items',
...
export const LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_ARCHIVE_HANDOFF_CLOSEOUT_STATUS = Object.freeze({
  local_only: true,
  deployment_status: 'BLOCKED_FOR_LIVE',
  pass_fail_status: 'PASS_LOCAL_ONLY',
  decision_evidence_archive_handoff_closeout_status: 'RESPONSE_DECISION_EVIDENCE_ARCHIVE_HANDOFF_CLOSEOUT_PENDING_EXTERNAL_RECORDS',
});

export const LOCAL_REPLAY_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_ARCHIVE_HANDOFF_CLOSEOUT_ITEMS = Object.freeze([
...
function assertNoSecretLookingValue(value, path) {
  if (typeof value === 'string' && SECRET_PATTERN.test(value)) {
    throw new Error(`Secret-looking value is not allowed in local replay approval decision external owner response decision evidence archive handoff closeout: ${path}`);
  }
...
export function createLocalReplayApprovalDecisionExternalOwnerResponseDecisionEvidenceArchiveHandoffCloseout(input) {
  if (!input || typeof input !== 'object' || Array.isArray(input)) {
    throw new Error('Local replay approval decision external owner response decision evidence archive handoff closeout input must be an object');
  }

  const handoff = input.approval_decision_external_owner_response_decision_evidence_archive_handoff;
  if (!handoff?.local_only) {
    throw new Error('Local replay approval decision external owner response decision evidence archive handoff closeout requires a local_only approval_decision_external_owner_response_decision_evidence_archive_handoff');
  }

  if (handoff.deployment_status !== 'BLOCKED_FOR_LIVE') {
    throw new Error('Local replay approval decision external owner response decision evidence archive handoff closeout handoff must be BLOCKED_FOR_LIVE');
  }

  if (handoff.pass_fail_status !== 'PASS_LOCAL_ONLY') {
    throw new Error('Local replay approval decision external owner response decision evidence archive handoff closeout handoff must be PASS_LOCAL_ONLY');
  }

  if (handoff.decision_evidence_archive_handoff_status !== 'RESPONSE_DECISION_EVIDENCE_ARCHIVE_HANDOFF_PENDING_EXTERNAL_RECORDS') {
    throw new Error('Local replay approval decision external owner response decision evidence archive handoff closeout requires RESPONSE_DECISION_EVIDENCE_ARCHIVE_HANDOFF_PENDING_EXTERNAL_RECORDS status');
  }

  if (!handoff.module_order?.includes('repayment_failure')) {
    throw new Error('Local replay approval decision external owner response decision evidence archive handoff closeout handoff module_order must include repayment_failure');
  }

  assertNoSecretLookingValue(input, 'local_replay_approval_decision_external_owner_response_decision_evidence_archive_handoff_closeout');

  const closeout = {
    approval_decision_external_owner_response_decision_evidence_archive_handoff_closeout_id: input.approval_decision_external_owner_response_decision_evidence_archive_handoff_closeout_id,
    ...
    source_decision_evidence_archive_handoff_status: handoff.decision_evidence_archive_handoff_status,
    closed_owner_review_handoff_items: LOCAL_REPLAY_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_ARCHIVE_HANDOFF_CLOSEOUT_ITEMS,
  ...
  for (const field of REQUIRED_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_ARCHIVE_HANDOFF_CLOSEOUT_FIELDS) {
    if (closeout[field] === undefined || closeout[field] === null || closeout[field] === '') {
      throw new Error(`Missing required local replay approval decision external owner response decision evidence archive handoff closeout field: ${field}`);
    }
  }
...
export const DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_ARCHIVE_HANDOFF_CLOSEOUT = Object.freeze(createLocalReplayApprovalDecisionExternalOwnerResponseDecisionEvidenceArchiveHandoffCloseout({
  approval_decision_external_owner_response_decision_evidence_archive_handoff_closeout_id: 'local_replay_approval_decision_external_owner_response_decision_evidence_archive_handoff_closeout_demo_001',
