...
[
  ...
  'Status: internal provider-response decision evidence closeout',
  'No provider response decision evidence closeout is recorded yet',
...
].forEach((phrase) => requirePhrase(decisionEvidenceCloseout, phrase, 'provider response decision evidence closeout'));
...
[
  ...
  'attorney reviewer',
].forEach((category) => requirePhrase(decisionEvidenceCloseout, category, 'provider response decision evidence closeout'));
...
].forEach((fileReference) => requirePhrase(decisionEvidenceCloseout, fileReference, 'provider response decision evidence closeout'));
...
requirePhrase(decisionEvidenceSummary, 'docs/whitepaper-v1-3-provider-response-decision-evidence-closeout.md', 'provider response decision evidence summary');
...
requirePhrase(decisionEvidenceIntake, 'docs/whitepaper-v1-3-provider-response-decision-evidence-closeout.md', 'provider response decision evidence intake');
...
requirePhrase(decisionEvidenceTemplate, 'docs/whitepaper-v1-3-provider-response-decision-evidence-closeout.md', 'provider response decision evidence template');
...
requirePhrase(decisionRegister, 'docs/whitepaper-v1-3-provider-response-decision-evidence-closeout.md', 'provider response decision register');
...
requirePhrase(intake, 'docs/whitepaper-v1-3-provider-response-decision-evidence-closeout.md', 'provider response intake template');
...
requirePhrase(routing, 'docs/whitepaper-v1-3-provider-response-decision-evidence-closeout.md', 'provider response routing checklist');
...
requirePhrase(evidenceLog, 'docs/whitepaper-v1-3-provider-response-decision-evidence-closeout.md', 'provider response evidence log');
...
requirePhrase(summaryShell, 'docs/whitepaper-v1-3-provider-response-decision-evidence-closeout.md', 'provider response summary shell');
...
requirePhrase(actionQueue, 'docs/whitepaper-v1-3-provider-response-decision-evidence-closeout.md', 'provider response action queue');
requirePhrase(publicationStatus, 'provider response decision evidence closeout | PENDING_PROVIDER_RESPONSE', 'publication evidence current status');
requirePhrase(founderReadyRollup, 'provider response decision evidence closeout | PENDING_PROVIDER_RESPONSE', 'founder-ready packet status rollup');
requirePhrase(masterIndex, 'provider response decision evidence closeout | local decision evidence closeout only / no provider response decision evidence closeout recorded', 'internal review master index');
...
[
  /\bprovider response decision evidence closeout recorded\b/i,
...
].forEach((pattern) => rejectPattern(decisionEvidenceCloseout, pattern, 'provider response decision evidence closeout'));

for (const [content, label] of [
  [publicWhitepaper, 'public whitepaper'],
  [publicHomepage, 'public homepage'],
]) {
  if (content.includes('Provider Response Decision Evidence Closeout') || content.includes('V13-PD-EVID-CLOSE-001')) {
    errors.push(`${label} appears to contain internal provider response decision evidence closeout content`);
  }
}

if (errors.length > 0) {
  console.error('whitepaper v1.3 provider response decision evidence closeout validation failed:');
  errors.forEach((error) => console.error(`- ${error}`));
  process.exit(1);
}

console.log('whitepaper v1.3 provider response decision evidence closeout validation passed');
