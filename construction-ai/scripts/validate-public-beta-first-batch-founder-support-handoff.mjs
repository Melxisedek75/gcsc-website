...
function fail(message) {
  console.error(`Public beta first batch founder support handoff validation failed: ${message}`);
  process.exit(1);
}
...
for (const required of [
  'redacted first-batch beta support handoff',
  'tester code',
  'safe issue ID',
  'safe request ID',
  'severity counts',
...
const scriptName = 'check:public-beta-first-batch-founder-support-handoff';

assertIncludes(context, 'Public beta first batch founder support handoff', contextPath);
assertIncludes(context, scriptName, contextPath);
assertIncludes(context, 'Backlog count at latest audit', contextPath);
assertIncludes(backlog, 'Public beta first batch founder support handoff', backlogPath);
assertIncludes(backlog, scriptName, backlogPath);
assertIncludes(audit, 'Public beta first batch founder support handoff', auditPath);
...
if (/https?:\/\/[^\s)`>"]+|sk_live_[a-z0-9]|-----BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY-----|xox[baprs]-[0-9]|service_role\s*[:=]|postgresql:\/\/|password\s*[:=]|eyJ[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}/i.test(handoff)) {
  fail('Public beta first batch founder support handoff must not contain real URLs or secret-looking values');
}

console.log(JSON.stringify({
  status: 'passed',
  public_beta_first_batch_founder_support_handoff: handoffPath,
  founder_review_handoff_checked: true,
