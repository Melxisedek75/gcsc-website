...
const recordPath = resolve('..', 'docs', 'smartcontractor-smart-contract-collateral-local-package-start-record.md');
const requiredDocPaths = [
  ...
  resolve('..', 'docs', 'smartcontractor-loan-legal-risk-model.md'),
...
function fail(message) {
  console.error(`Smart contract collateral local package start validation failed: ${message}`);
  process.exit(1);
}
...
for (const required of [
  ...
  'WP-COLLATERAL-LOCAL',
  'collateral state constants',
  'token estimate fixture planning',
  'oracle snapshot placeholder planning',
  'LTV label fixture planning',
  ...
  'real_token_lock_checked',
  'custody_provider_checked',
  'oracle_provider_checked',
  'margin_call_checked',
  'token_price_appreciation_claim_checked',
  'no secrets allowed',
...
assertIncludes(context, 'Smart contract collateral local package start', contextPath);
assertIncludes(context, 'check:smart-contract-collateral-local-package-start', contextPath);
assertIncludes(backlog, 'Smart contract collateral local package start', backlogPath);
assertIncludes(backlog, 'check:smart-contract-collateral-local-package-start', backlogPath);
assertIncludes(realAudit, 'Smart contract collateral local package start', realAuditPath);

const scriptName = 'check:smart-contract-collateral-local-package-start';
...
if (/sk_live_[a-z0-9]|-----BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY-----|xox[baprs]-[0-9]|service_role\s*[:=]|postgresql:\/\/|password\s*[:=]|eyJ[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}/i.test(record)) {
  fail('Smart contract collateral local package start record must not contain real secret-looking values');
}

console.log(JSON.stringify({
  status: 'passed',
  smart_contract_collateral_local_package_start_record: recordPath,
  collateral_local_package_start_checked: true,
}, null, 2));
