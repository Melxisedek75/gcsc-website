...
function fail(message) {
  console.error(`Public beta first batch support trend archive index validation failed: ${message}`);
  process.exit(1);
}
...
for (const section of [
  ...
  'Required Checks',
  'Acceptance Check',
]) assertIncludes(indexDoc, section, indexPath);

for (const required of [
  'redacted support trend archive index',
  'support trend internal action archive',
  'internal archive index metadata',
  'tester-code-only',
  'safe issue IDs',
  'safe request IDs',
  'queue item IDs',
  ...
  'npm run check',
]) assertIncludes(indexDoc, required, indexPath);
...
const scriptName = 'check:public-beta-first-batch-support-trend-archive-index';

assertIncludes(context, 'Public beta first batch support trend archive index', contextPath);
assertIncludes(context, scriptName, contextPath);
assertIncludes(context, 'Backlog count at latest audit', contextPath);
assertIncludes(backlog, 'Public beta first batch support trend archive index', backlogPath);
assertIncludes(backlog, scriptName, backlogPath);
assertIncludes(audit, 'Public beta first batch support trend archive index', auditPath);
assertIncludes(packageJson, `"${scriptName}"`, packagePath);
assertIncludes(runner, `"${scriptName}"`, runnerPath);

if (/https?:\/\/[^\s)`>"]+|sk_live_[a-z0-9]|-----BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY-----|xox[baprs]-[0-9]|service_role\s*[:=]|postgresql:\/\/|password\s*[:=]|eyJ[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}/i.test(indexDoc)) {
  fail('Public beta first batch support trend archive index must not contain real URLs or secret-looking values');
}

console.log(JSON.stringify({
  status: 'passed',
  public_beta_first_batch_support_trend_archive_index: indexPath,
  trend_archive_index_checked: true,
