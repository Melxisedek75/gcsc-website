...
const recordPath = resolve('..', 'docs', 'smartcontractor-smart-contract-authority-local-package-start-record.md');
...
function fail(message) {
  console.error(`Smart contract authority local package start validation failed: ${message}`);
  process.exit(1);
}
...
for (const required of [
  ...
  'single_key_production_authority_checked',
  'developer_only_owner_authority_checked',
  'production_permission_change_checked',
  'no secrets allowed',
...
assertIncludes(context, 'Smart contract authority local package start', contextPath);
assertIncludes(context, 'check:smart-contract-authority-local-package-start', contextPath);
assertIncludes(backlog, 'Smart contract authority local package start', backlogPath);
assertIncludes(backlog, 'check:smart-contract-authority-local-package-start', backlogPath);
assertIncludes(realAudit, 'Smart contract authority local package start', realAuditPath);

const scriptName = 'check:smart-contract-authority-local-package-start';
...
if (/sk_live_[a-z0-9]|-----BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY-----|xox[baprs]-[0-9]|service_role\s*[:=]|postgresql:\/\/|password\s*[:=]|eyJ[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}/i.test(record)) {
  fail('Smart contract authority local package start record must not contain real secret-looking values');
}

console.log(JSON.stringify({
  status: 'passed',
  smart_contract_authority_local_package_start_record: recordPath,
  authority_local_package_start_checked: true,
}, null, 2));
