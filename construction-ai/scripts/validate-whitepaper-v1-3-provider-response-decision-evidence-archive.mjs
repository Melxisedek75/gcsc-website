...
[
  ...
  'Status: internal provider-response decision evidence archive',
  'No provider response decision evidence archive is recorded yet',
...
].forEach((phrase) => requirePhrase(decisionEvidenceArchive, phrase, 'provider response decision evidence archive'));
...
[
  ...
  'attorney reviewer',
].forEach((category) => requirePhrase(decisionEvidenceArchive, category, 'provider response decision evidence archive'));
...
].forEach((fileReference) => requirePhrase(decisionEvidenceArchive, fileReference, 'provider response decision evidence archive'));
...
requirePhrase(decisionEvidenceCloseout, 'docs/whitepaper-v1-3-provider-response-decision-evidence-archive.md', 'provider response decision evidence closeout');
...
requirePhrase(decisionEvidenceSummary, 'docs/whitepaper-v1-3-provider-response-decision-evidence-archive.md', 'provider response decision evidence summary');
...
requirePhrase(decisionEvidenceIntake, 'docs/whitepaper-v1-3-provider-response-decision-evidence-archive.md', 'provider response decision evidence intake');
...
requirePhrase(decisionEvidenceTemplate, 'docs/whitepaper-v1-3-provider-response-decision-evidence-archive.md', 'provider response decision evidence template');
...
requirePhrase(decisionRegister, 'docs/whitepaper-v1-3-provider-response-decision-evidence-archive.md', 'provider response decision register');
...
requirePhrase(intake, 'docs/whitepaper-v1-3-provider-response-decision-evidence-archive.md', 'provider response intake template');
...
requirePhrase(routing, 'docs/whitepaper-v1-3-provider-response-decision-evidence-archive.md', 'provider response routing checklist');
...
requirePhrase(evidenceLog, 'docs/whitepaper-v1-3-provider-response-decision-evidence-archive.md', 'provider response evidence log');
...
requirePhrase(summaryShell, 'docs/whitepaper-v1-3-provider-response-decision-evidence-archive.md', 'provider response summary shell');
...
requirePhrase(actionQueue, 'docs/whitepaper-v1-3-provider-response-decision-evidence-archive.md', 'provider response action queue');
requirePhrase(publicationStatus, 'provider response decision evidence archive | PENDING_PROVIDER_RESPONSE', 'publication evidence current status');
requirePhrase(founderReadyRollup, 'provider response decision evidence archive | PENDING_PROVIDER_RESPONSE', 'founder-ready packet status rollup');
requirePhrase(masterIndex, 'provider response decision evidence archive | local decision evidence archive only / no provider response decision evidence archive recorded', 'internal review master index');
...
[
  /\bprovider response decision evidence archive recorded\b/i,
...
].forEach((pattern) => rejectPattern(decisionEvidenceArchive, pattern, 'provider response decision evidence archive'));

for (const [content, label] of [
  [publicWhitepaper, 'public whitepaper'],
  [publicHomepage, 'public homepage'],
]) {
  if (content.includes('Provider Response Decision Evidence Archive') || content.includes('V13-PD-EVID-ARCH-001')) {
    errors.push(`${label} appears to contain internal provider response decision evidence archive content`);
  }
}

if (errors.length > 0) {
  console.error('whitepaper v1.3 provider response decision evidence archive validation failed:');
  errors.forEach((error) => console.error(`- ${error}`));
  process.exit(1);
}

console.log('whitepaper v1.3 provider response decision evidence archive validation passed');
