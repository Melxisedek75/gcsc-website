...
const ownershipPath = resolve('..', 'docs', 'smartcontractor-smart-contract-code-ownership-plan.md');
...
function fail(message) {
  console.error(`Smart contract code ownership validation failed: ${message}`);
  process.exit(1);
}
...
const ownership = readRequired(ownershipPath);
...
for (const section of [
  ...
  'Required Links',
  'Required Checks',
]) assertIncludes(ownership, section, ownershipPath);

for (const required of [
  'internal code ownership plan only',
  ...
  'npm run check',
]) assertIncludes(ownership, required, ownershipPath);

assertIncludes(context, 'Smart contract code ownership', contextPath);
assertIncludes(context, 'check:smart-contract-code-ownership', contextPath);
assertIncludes(backlog, 'Smart contract code ownership', backlogPath);
assertIncludes(backlog, 'check:smart-contract-code-ownership', backlogPath);
assertIncludes(realAudit, 'Smart contract code ownership', realAuditPath);

const scriptName = 'check:smart-contract-code-ownership';
...
if (/sk_live_[a-z0-9]|-----BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY-----|xox[baprs]-[0-9]|service_role\s*[:=]|postgresql:\/\/|password\s*[:=]|eyJ[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}/i.test(ownership)) {
  fail('Smart contract code ownership plan must not contain real secret-looking values');
}

console.log(JSON.stringify({
  status: 'passed',
  smart_contract_code_ownership_plan: ownershipPath,
  code_ownership_checked: true,
}, null, 2));
