...
const files = {
  externalRecordRequestCloseoutHandoff: path.join(root, 'docs', 'whitepaper-v1-3-provider-response-decision-evidence-archive-external-record-request-closeout-handoff.md'),
...
const errors = [];
const handoffReference = 'docs/whitepaper-v1-3-provider-response-decision-evidence-archive-external-record-request-closeout-handoff.md';
...
const externalRecordRequestCloseoutHandoff = readRequired('provider response decision evidence archive external record request closeout handoff', files.externalRecordRequestCloseoutHandoff);
const publicWhitepaper = readRequired('public whitepaper', files.publicWhitepaper);
const publicHomepage = readRequired('public homepage', files.publicHomepage);

[
  'Provider Response Decision Evidence Archive External Record Request Closeout Handoff',
  'Status: internal provider-response decision evidence archive external record request closeout handoff',
  'No provider response decision evidence archive external record request closeout handoff is recorded yet',
...
].forEach((phrase) => requirePhrase(externalRecordRequestCloseoutHandoff, phrase, 'provider response decision evidence archive external record request closeout handoff'));
...
[
  ...
  'security reviewer',
  'product owner',
].forEach((category) => requirePhrase(externalRecordRequestCloseoutHandoff, category, 'provider response decision evidence archive external record request closeout handoff'));
...
].forEach((fileReference) => requirePhrase(externalRecordRequestCloseoutHandoff, fileReference, 'provider response decision evidence archive external record request closeout handoff'));

for (const [label, filePath] of Object.entries(files)) {
  if (['externalRecordRequestCloseoutHandoff', 'publicationStatus', 'founderReadyRollup', 'masterIndex', 'publicWhitepaper', 'publicHomepage'].includes(label)) continue;
  const content = readRequired(label, filePath);
  requirePhrase(content, handoffReference, label);
}
...
requirePhrase(publicationStatus, 'provider response decision evidence archive external record request closeout handoff | PENDING_FOUNDER_REQUEST_SCOPE', 'publication evidence current status');
requirePhrase(founderReadyRollup, 'provider response decision evidence archive external record request closeout handoff | PENDING_FOUNDER_REQUEST_SCOPE', 'founder-ready packet status rollup');
requirePhrase(masterIndex, 'provider response decision evidence archive external record request closeout handoff | local external record request closeout handoff template only / no provider response decision evidence archive external record request closeout handoff recorded', 'internal review master index');

[
  /\bprovider response decision evidence archive external record request closeout handoff approved\b/i,
...
].forEach((pattern) => rejectPattern(externalRecordRequestCloseoutHandoff, pattern, 'provider response decision evidence archive external record request closeout handoff'));

for (const [content, label] of [
  [publicWhitepaper, 'public whitepaper'],
  [publicHomepage, 'public homepage'],
]) {
  if (content.includes('Provider Response Decision Evidence Archive External Record Request Closeout Handoff') || content.includes('V13-PD-EVID-ARCH-EXTREQ-CLOSE-HANDOFF-001')) {
    errors.push(`${label} appears to contain internal provider response decision evidence archive external record request closeout handoff content`);
  }
}

if (errors.length > 0) {
  console.error('whitepaper v1.3 provider response decision evidence archive external record request closeout handoff validation failed:');
  errors.forEach((error) => console.error(`- ${error}`));
  process.exit(1);
}

console.log('whitepaper v1.3 provider response decision evidence archive external record request closeout handoff validation passed');
