...
function fail(message) {
  console.error(`Public beta first batch support trend archive index internal action closeout handoff validation failed: ${message}`);
  process.exit(1);
}
...
for (const section of [
  ...
  'Required Checks',
  'Acceptance Check',
]) assertIncludes(closeoutHandoffDoc, section, closeoutHandoffDocPath);

for (const required of [
  'redacted support trend archive index internal action closeout handoff records',
  'support trend archive index internal action closeout',
  'internal archive index action closeout handoff metadata',
  'founder decision option',
  'founder-review-ready',
  'internal-action-closeout-handoff-ready',
  'tester-code-only',
  'safe issue IDs',
  'safe request IDs',
  'queue item IDs',
  ...
  'npm run check',
]) assertIncludes(closeoutHandoffDoc, required, closeoutHandoffDocPath);
...
const scriptName = 'check:public-beta-first-batch-support-trend-archive-index-internal-action-closeout-handoff';

assertIncludes(context, 'Public beta first batch support trend archive index internal action closeout handoff', contextPath);
...
assertIncludes(backlog, 'Public beta first batch support trend archive index internal action closeout handoff', backlogPath);
assertIncludes(backlog, scriptName, backlogPath);
assertIncludes(audit, 'Public beta first batch support trend archive index internal action closeout handoff', auditPath);
...
if (/https?:\/\/[^\s)`>"]+|sk_live_[a-z0-9]|-----BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY-----|xox[baprs]-[0-9]|service_role\s*[:=]|postgresql:\/\/|password\s*[:=]|eyJ[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}/i.test(closeoutHandoffDoc)) {
  fail('Public beta first batch support trend archive index internal action closeout handoff must not contain real URLs or secret-looking values');
}

console.log(JSON.stringify({
  status: 'passed',
  public_beta_first_batch_support_trend_archive_index_internal_action_closeout_handoff: closeoutHandoffDocPath,
  trend_archive_index_action_closeout_handoff_checked: true,
