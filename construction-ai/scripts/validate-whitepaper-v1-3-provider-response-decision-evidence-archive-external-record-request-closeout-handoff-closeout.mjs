...
const files = {
  externalRecordRequestCloseoutHandoffCloseout: path.join(root, 'docs', 'whitepaper-v1-3-provider-response-decision-evidence-archive-external-record-request-closeout-handoff-closeout.md'),
...
const errors = [];
const closeoutReference = 'docs/whitepaper-v1-3-provider-response-decision-evidence-archive-external-record-request-closeout-handoff-closeout.md';
...
const closeout = readRequired('provider response decision evidence archive external record request closeout handoff closeout', files.externalRecordRequestCloseoutHandoffCloseout);
const publicWhitepaper = readRequired('public whitepaper', files.publicWhitepaper);
const publicHomepage = readRequired('public homepage', files.publicHomepage);

[
  'Provider Response Decision Evidence Archive External Record Request Closeout Handoff Closeout',
  'Status: internal provider-response decision evidence archive external record request closeout handoff closeout',
  'No provider response decision evidence archive external record request closeout handoff closeout is recorded yet',
...
].forEach((phrase) => requirePhrase(closeout, phrase, 'provider response decision evidence archive external record request closeout handoff closeout'));
...
[
  ...
  'security reviewer',
  'product owner',
].forEach((category) => requirePhrase(closeout, category, 'provider response decision evidence archive external record request closeout handoff closeout'));
...
].forEach((fileReference) => requirePhrase(closeout, fileReference, 'provider response decision evidence archive external record request closeout handoff closeout'));

for (const [label, filePath] of Object.entries(files)) {
  if (['externalRecordRequestCloseoutHandoffCloseout', 'publicationStatus', 'founderReadyRollup', 'masterIndex', 'publicWhitepaper', 'publicHomepage'].includes(label)) continue;
  const content = readRequired(label, filePath);
  requirePhrase(content, closeoutReference, label);
}
...
requirePhrase(publicationStatus, 'provider response decision evidence archive external record request closeout handoff closeout | PENDING_FOUNDER_REQUEST_SCOPE', 'publication evidence current status');
requirePhrase(founderReadyRollup, 'provider response decision evidence archive external record request closeout handoff closeout | PENDING_FOUNDER_REQUEST_SCOPE', 'founder-ready packet status rollup');
requirePhrase(masterIndex, 'provider response decision evidence archive external record request closeout handoff closeout | local external record request closeout handoff closeout template only / no provider response decision evidence archive external record request closeout handoff closeout recorded', 'internal review master index');

[
  /\bprovider response decision evidence archive external record request closeout handoff closeout approved\b/i,
...
].forEach((pattern) => rejectPattern(closeout, pattern, 'provider response decision evidence archive external record request closeout handoff closeout'));

for (const [content, label] of [
  [publicWhitepaper, 'public whitepaper'],
  [publicHomepage, 'public homepage'],
]) {
  if (content.includes('Provider Response Decision Evidence Archive External Record Request Closeout Handoff Closeout') || content.includes('V13-PD-EVID-ARCH-EXTREQ-CLOSE-HANDOFF-CLOSE-001')) {
    errors.push(`${label} appears to contain internal provider response decision evidence archive external record request closeout handoff closeout content`);
  }
}

if (errors.length > 0) {
  console.error('whitepaper v1.3 provider response decision evidence archive external record request closeout handoff closeout validation failed:');
  errors.forEach((error) => console.error(`- ${error}`));
  process.exit(1);
}

console.log('whitepaper v1.3 provider response decision evidence archive external record request closeout handoff closeout validation passed');
