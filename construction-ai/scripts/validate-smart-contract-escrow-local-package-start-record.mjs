...
const recordPath = resolve('..', 'docs', 'smartcontractor-smart-contract-escrow-local-package-start-record.md');
...
function fail(message) {
  console.error(`Smart contract escrow local package start validation failed: ${message}`);
  process.exit(1);
}
...
for (const required of [
  ...
  'project-contract escrow state planning',
  'demo release-condition fixtures',
  ...
  'automatic_payment_release_checked',
  'escrow_agent_claim_checked',
  'provider_money_movement_checked',
  'no secrets allowed',
...
assertIncludes(context, 'Smart contract escrow local package start', contextPath);
assertIncludes(context, 'check:smart-contract-escrow-local-package-start', contextPath);
assertIncludes(backlog, 'Smart contract escrow local package start', backlogPath);
assertIncludes(backlog, 'check:smart-contract-escrow-local-package-start', backlogPath);
assertIncludes(realAudit, 'Smart contract escrow local package start', realAuditPath);

const scriptName = 'check:smart-contract-escrow-local-package-start';
...
if (/sk_live_[a-z0-9]|-----BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY-----|xox[baprs]-[0-9]|service_role\s*[:=]|postgresql:\/\/|password\s*[:=]|eyJ[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}/i.test(record)) {
  fail('Smart contract escrow local package start record must not contain real secret-looking values');
}

console.log(JSON.stringify({
  status: 'passed',
  smart_contract_escrow_local_package_start_record: recordPath,
  escrow_local_package_start_checked: true,
}, null, 2));
