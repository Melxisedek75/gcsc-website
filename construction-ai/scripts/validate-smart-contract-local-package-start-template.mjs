...
const templatePath = resolve('..', 'docs', 'smartcontractor-smart-contract-local-package-start-template.md');
...
function fail(message) {
  console.error(`Smart contract local package start template validation failed: ${message}`);
  process.exit(1);
}
...
for (const required of [
  'internal local package start template only',
...
assertIncludes(context, 'Smart contract local package start template', contextPath);
assertIncludes(context, 'check:smart-contract-local-package-start-template', contextPath);
assertIncludes(backlog, 'Smart contract local package start template', backlogPath);
assertIncludes(backlog, 'check:smart-contract-local-package-start-template', backlogPath);
assertIncludes(realAudit, 'Smart contract local package start template', realAuditPath);

const scriptName = 'check:smart-contract-local-package-start-template';
...
if (/sk_live_[a-z0-9]|-----BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY-----|xox[baprs]-[0-9]|service_role\s*[:=]|postgresql:\/\/|password\s*[:=]|eyJ[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}/i.test(template)) {
  fail('Smart contract local package start template must not contain real secret-looking values');
}

console.log(JSON.stringify({
  status: 'passed',
  smart_contract_local_package_start_template: templatePath,
  local_package_start_template_checked: true,
}, null, 2));
