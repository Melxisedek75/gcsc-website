...
if (smartContracts.DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_REGISTER_CLOSEOUT.local_only !== true) {
  fail('Demo replay approval decision external owner response decision register closeout export must stay local_only');
}
...
if (smartContracts.DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_INTAKE.local_only !== true) {
  fail('Demo replay approval decision external owner response decision evidence intake export must stay local_only');
}
if (smartContracts.DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_INTAKE.deployment_status !== 'BLOCKED_FOR_LIVE') {
  fail('Demo replay approval decision external owner response decision evidence intake export must stay BLOCKED_FOR_LIVE');
}
if (smartContracts.DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_SUMMARY.local_only !== true) {
  fail('Demo replay approval decision external owner response decision evidence summary export must stay local_only');
}
if (smartContracts.DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_SUMMARY.deployment_status !== 'BLOCKED_FOR_LIVE') {
  fail('Demo replay approval decision external owner response decision evidence summary export must stay BLOCKED_FOR_LIVE');
}
if (smartContracts.DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_CLOSEOUT.local_only !== true) {
  fail('Demo replay approval decision external owner response decision evidence closeout export must stay local_only');
}
if (smartContracts.DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_CLOSEOUT.deployment_status !== 'BLOCKED_FOR_LIVE') {
  fail('Demo replay approval decision external owner response decision evidence closeout export must stay BLOCKED_FOR_LIVE');
}
if (smartContracts.DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_ARCHIVE.local_only !== true) {
  fail('Demo replay approval decision external owner response decision evidence archive export must stay local_only');
}
if (smartContracts.DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_ARCHIVE.deployment_status !== 'BLOCKED_FOR_LIVE') {
  fail('Demo replay approval decision external owner response decision evidence archive export must stay BLOCKED_FOR_LIVE');
}
if (smartContracts.DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_ARCHIVE_INDEX.local_only !== true) {
  fail('Demo replay approval decision external owner response decision evidence archive index export must stay local_only');
}
if (smartContracts.DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_ARCHIVE_INDEX.deployment_status !== 'BLOCKED_FOR_LIVE') {
  fail('Demo replay approval decision external owner response decision evidence archive index export must stay BLOCKED_FOR_LIVE');
}
if (smartContracts.DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_ARCHIVE_INDEX_CLOSEOUT.local_only !== true) {
  fail('Demo replay approval decision external owner response decision evidence archive index closeout export must stay local_only');
}
...
if (smartContracts.DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_ARCHIVE_HANDOFF_CLOSEOUT.local_only !== true) {
  fail('Demo replay approval decision external owner response decision evidence archive handoff closeout export must stay local_only');
}
if (smartContracts.DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_ARCHIVE_HANDOFF_CLOSEOUT.deployment_status !== 'BLOCKED_FOR_LIVE') {
  fail('Demo replay approval decision external owner response decision evidence archive handoff closeout export must stay BLOCKED_FOR_LIVE');
}
if (smartContracts.DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_ARCHIVE_EXTERNAL_RECORD_REQUEST.local_only !== true) {
  fail('Demo replay approval decision external owner response decision evidence archive external record request export must stay local_only');
}
if (smartContracts.DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_ARCHIVE_EXTERNAL_RECORD_REQUEST.deployment_status !== 'BLOCKED_FOR_LIVE') {
  fail('Demo replay approval decision external owner response decision evidence archive external record request export must stay BLOCKED_FOR_LIVE');
}
if (smartContracts.DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_ARCHIVE_EXTERNAL_RECORD_REQUEST_CLOSEOUT.local_only !== true) {
  fail('Demo replay approval decision external owner response decision evidence archive external record request closeout export must stay local_only');
}
if (smartContracts.DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_ARCHIVE_EXTERNAL_RECORD_REQUEST_CLOSEOUT.deployment_status !== 'BLOCKED_FOR_LIVE') {
  fail('Demo replay approval decision external owner response decision evidence archive external record request closeout export must stay BLOCKED_FOR_LIVE');
}
if (smartContracts.DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_ARCHIVE_EXTERNAL_RECORD_REQUEST_CLOSEOUT_HANDOFF.local_only !== true) {
  fail('Demo replay approval decision external owner response decision evidence archive external record request closeout handoff export must stay local_only');
}
if (smartContracts.DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_ARCHIVE_EXTERNAL_RECORD_REQUEST_CLOSEOUT_HANDOFF.deployment_status !== 'BLOCKED_FOR_LIVE') {
  fail('Demo replay approval decision external owner response decision evidence archive external record request closeout handoff export must stay BLOCKED_FOR_LIVE');
}
if (smartContracts.DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_ARCHIVE_EXTERNAL_RECORD_REQUEST_CLOSEOUT_HANDOFF_CLOSEOUT.local_only !== true) {
  fail('Demo replay approval decision external owner response decision evidence archive external record request closeout handoff closeout export must stay local_only');
}
if (smartContracts.DEMO_LOCAL_REPLAY_APPROVAL_DECISION_EXTERNAL_OWNER_RESPONSE_DECISION_EVIDENCE_ARCHIVE_EXTERNAL_RECORD_REQUEST_CLOSEOUT_HANDOFF_CLOSEOUT.deployment_status !== 'BLOCKED_FOR_LIVE') {
  fail('Demo replay approval decision external owner response decision evidence archive external record request closeout handoff closeout export must stay BLOCKED_FOR_LIVE');
}
...
for (const requiredSource of [
  ...
  './replay/localReplayApprovalDecisionExternalOwnerResponseDecisionEvidenceArchiveHandoffCloseout.mjs',
  './replay/localReplayApprovalDecisionExternalOwnerResponseDecisionEvidenceArchiveExternalRecordRequest.mjs',
  './replay/localReplayApprovalDecisionExternalOwnerResponseDecisionEvidenceArchiveExternalRecordRequestCloseout.mjs',
  './replay/localReplayApprovalDecisionExternalOwnerResponseDecisionEvidenceArchiveExternalRecordRequestCloseoutHandoff.mjs',
  './replay/localReplayApprovalDecisionExternalOwnerResponseDecisionEvidenceArchiveExternalRecordRequestCloseoutHandoffCloseout.mjs',
