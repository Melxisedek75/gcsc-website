...
[
  'Provider Response Decision Evidence Archive External Record Request',
  'Status: internal provider-response decision evidence archive external record request template',
  'No provider response decision evidence archive external record request is recorded or sent yet',
...
].forEach((phrase) => requirePhrase(externalRecordRequest, phrase, 'provider response decision evidence archive external record request'));
...
[
  ...
  'attorney reviewer',
].forEach((category) => requirePhrase(externalRecordRequest, category, 'provider response decision evidence archive external record request'));
...
].forEach((fileReference) => requirePhrase(externalRecordRequest, fileReference, 'provider response decision evidence archive external record request'));
...
[
  ...
  requirePhrase(content, 'docs/whitepaper-v1-3-provider-response-decision-evidence-archive-external-record-request.md', label);
});

requirePhrase(publicationStatus, 'provider response decision evidence archive external record request | PENDING_FOUNDER_REQUEST_SCOPE', 'publication evidence current status');
requirePhrase(founderReadyRollup, 'provider response decision evidence archive external record request | PENDING_FOUNDER_REQUEST_SCOPE', 'founder-ready packet status rollup');
requirePhrase(masterIndex, 'provider response decision evidence archive external record request | local external record request template only / no provider response decision evidence archive external record request recorded or sent', 'internal review master index');
...
[
  /\bprovider response decision evidence archive external record request approved\b/i,
...
].forEach((pattern) => rejectPattern(externalRecordRequest, pattern, 'provider response decision evidence archive external record request'));

for (const [content, label] of [
  [publicWhitepaper, 'public whitepaper'],
  [publicHomepage, 'public homepage'],
]) {
  if (content.includes('Provider Response Decision Evidence Archive External Record Request') || content.includes('V13-PD-EVID-ARCH-EXTREQ-001')) {
    errors.push(`${label} appears to contain internal provider response decision evidence archive external record request content`);
  }
}

if (errors.length > 0) {
  console.error('whitepaper v1.3 provider response decision evidence archive external record request validation failed:');
  errors.forEach((error) => console.error(`- ${error}`));
  process.exit(1);
}

console.log('whitepaper v1.3 provider response decision evidence archive external record request validation passed');
