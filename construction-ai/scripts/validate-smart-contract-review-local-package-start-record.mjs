...
const recordPath = resolve('..', 'docs', 'smartcontractor-smart-contract-review-local-package-start-record.md');
const requiredDocPaths = [
  resolve('..', 'docs', 'smartcontractor-smart-contract-audit-local-package-start-record.md'),
  resolve('..', 'docs', 'smartcontractor-smart-contract-authority-local-package-start-record.md'),
  resolve('..', 'docs', 'smartcontractor-smart-contract-escrow-local-package-start-record.md'),
  resolve('..', 'docs', 'smartcontractor-smart-contract-loan-local-package-start-record.md'),
  resolve('..', 'docs', 'smartcontractor-smart-contract-collateral-local-package-start-record.md'),
  resolve('..', 'docs', 'smartcontractor-smart-contract-design.md'),
  resolve('..', 'docs', 'upwork-research-smartcontractor-disputes.md'),
...
function fail(message) {
  console.error(`Smart contract review local package start validation failed: ${message}`);
  process.exit(1);
}
...
for (const required of [
  ...
  'does not approve real token collateral',
  ...
  'does not approve AI or peer review as final payment-release authority',
  'WP-REVIEW-LOCAL',
  'peer review state constants',
  'review metadata fixture planning',
  'reputation-point fixture planning',
  'reward-payable placeholder planning',
  'conflict-of-interest fixture planning',
  'deterministic no-real-money replay',
  ...
  'escrow_fixture_status',
  'loan_fixture_status',
  'collateral_fixture_status',
  ...
  'peer_review_final_authority_checked',
  'real_reward_payout_checked',
  'token_issuance_checked',
  'public_reputation_claims_checked',
  'secrets_checked',
...
assertIncludes(context, 'Smart contract review local package start', contextPath);
assertIncludes(context, 'check:smart-contract-review-local-package-start', contextPath);
assertIncludes(backlog, 'Smart contract review local package start', backlogPath);
assertIncludes(backlog, 'check:smart-contract-review-local-package-start', backlogPath);
assertIncludes(realAudit, 'Smart contract review local package start', realAuditPath);

const scriptName = 'check:smart-contract-review-local-package-start';
...
if (/sk_live_[a-z0-9]|-----BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY-----|xox[baprs]-[0-9]|service_role\s*[:=]|postgresql:\/\/|password\s*[:=]|eyJ[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}/i.test(record)) {
  fail('Smart contract review local package start record must not contain real secret-looking values');
}

console.log(JSON.stringify({
  status: 'passed',
  smart_contract_review_local_package_start_record: recordPath,
  review_local_package_start_checked: true,
}, null, 2));
