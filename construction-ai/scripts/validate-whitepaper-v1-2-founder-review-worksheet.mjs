import { existsSync, readFileSync } from 'node:fs';
import { resolve } from 'node:path';

const worksheetPath = resolve('..', 'docs', 'whitepaper-v1-2-founder-review-worksheet.md');
const referencedDocs = [
  'whitepaper-v1-2-section-replacement-preview.md',
  'whitepaper-v1-2-redline-preview.md',
  'whitepaper-v1-2-founder-approval-brief.md',
  'whitepaper-v1-2-public-edit-queue.md',
  'whitepaper-v1-2-claim-review-matrix.md',
  'whitepaper-v1-2-terms-glossary.md',
  'whitepaper-v1-2-public-excerpt-guard.md',
  'whitepaper-v1-2-publish-gate.md',
  'whitepaper-v1-2-approval-record-template.md',
];
const contextPath = resolve('..', 'docs', 'gcsc-active-context.md');
const backlogPath = resolve('..', 'docs', 'smartcontractor-backlog.md');

function fail(message) {
  console.error(`Whitepaper v1.2 founder review worksheet validation failed: ${message}`);
  process.exit(1);
}

function readRequired(path) {
  if (!existsSync(path)) fail(`Missing required file: ${path}`);
  return readFileSync(path, 'utf8');
}

function assertIncludes(content, snippet, file) {
  if (!content.toLowerCase().includes(snippet.toLowerCase())) fail(`${file} must include: ${snippet}`);
}

const worksheet = readRequired(worksheetPath);
const context = readRequired(contextPath);
const backlog = readRequired(backlogPath);

for (const section of [
  'GCSC Whitepaper v1.2 Founder Review Worksheet',
  'Purpose',
  'How To Use',
  'Founder Review Items',
  'Wording Review',
  'Must Stay Blocked',
  'Approval Record Needed',
  'Required Commands',
  'Safe Default',
]) {
  assertIncludes(worksheet, section, worksheetPath);
}

for (const required of [
  'internal founder review worksheet',
  'not approval to publish or edit',
  'Accept',
  'Revise',
  'Reject',
  'SmartContractor Marketplace',
  'project contracts and milestones',
  'Contractor Reputation Layer',
  'AI-assisted workflows',
  'AI boundaries',
  'Escrow-ready and credit-ready',
  'Stablecoin settlement roadmap',
  'Tokenized construction agreements',
  'Digital Asset Market Clarity Act',
  'policy context, not a legal conclusion',
  'Real Estate DAO',
  'GCSC/GCST utility',
  'real escrow is live',
  'real lending is live',
  'real token collateral is live',
  'contractor credit is guaranteed',
  'token price is promised',
  'yield is guaranteed',
  'AI makes automatic legal or financial decisions',
  'attorney/provider/founder approval is optional',
  'founder approval recorded',
  'strict RLS/admin readiness',
  'disabled real-money gates',
  'whitepaper.html',
  'npm run check:whitepaper-v1-2-founder-review-worksheet',
  'npm run check:whitepaper-v1-2-section-replacement-preview',
  'npm run check:whitepaper-v1-2-claim-review',
  'npm run check',
]) {
  assertIncludes(worksheet, required, worksheetPath);
}

for (const docPath of referencedDocs) {
  assertIncludes(worksheet, docPath, worksheetPath);
  if (!existsSync(resolve('..', 'docs', docPath))) fail(`Referenced document must exist: ${docPath}`);
}

assertIncludes(context, 'whitepaper v1.2 founder review worksheet', contextPath);
assertIncludes(context, 'check:whitepaper-v1-2-founder-review-worksheet', contextPath);
assertIncludes(backlog, 'Whitepaper v1.2 founder review worksheet', backlogPath);
assertIncludes(backlog, 'check:whitepaper-v1-2-founder-review-worksheet', backlogPath);

if (/sk_live_[a-z0-9]|-----BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY-----|xox[baprs]-[0-9]|service_role\s*[:=]|postgresql:\/\/|password\s*[:=]|eyJ[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}/i.test(worksheet)) {
  fail('Founder review worksheet must not contain real secret-looking values');
}

console.log(JSON.stringify({ status: 'passed', founder_review_worksheet: worksheetPath, public_edit_block_checked: true }, null, 2));
