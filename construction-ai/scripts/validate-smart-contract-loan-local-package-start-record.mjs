...
const recordPath = resolve('..', 'docs', 'smartcontractor-smart-contract-loan-local-package-start-record.md');
const requiredDocPaths = [
  ...
  resolve('..', 'docs', 'whitepaper-v1-2-contract-backed-loan-implementation-readiness-matrix.md'),
...
function fail(message) {
  console.error(`Smart contract loan local package start validation failed: ${message}`);
  process.exit(1);
}
...
for (const required of [
  ...
  'signed-contract receivables fixture planning',
  'repayment-first waterfall fixture planning',
  ...
  'loan_origination_checked',
  'provider_underwriting_checked',
  'borrower_obligation_checked',
  'lender_or_bank_claim_checked',
  'no secrets allowed',
...
assertIncludes(context, 'Smart contract loan local package start', contextPath);
assertIncludes(context, 'check:smart-contract-loan-local-package-start', contextPath);
assertIncludes(backlog, 'Smart contract loan local package start', backlogPath);
assertIncludes(backlog, 'check:smart-contract-loan-local-package-start', backlogPath);
assertIncludes(realAudit, 'Smart contract loan local package start', realAuditPath);

const scriptName = 'check:smart-contract-loan-local-package-start';
...
if (/sk_live_[a-z0-9]|-----BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY-----|xox[baprs]-[0-9]|service_role\s*[:=]|postgresql:\/\/|password\s*[:=]|eyJ[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}/i.test(record)) {
  fail('Smart contract loan local package start record must not contain real secret-looking values');
}

console.log(JSON.stringify({
  status: 'passed',
  smart_contract_loan_local_package_start_record: recordPath,
  loan_local_package_start_checked: true,
}, null, 2));
