...
const docPath = resolve('..', 'docs', 'smartcontractor-public-beta-week-two-kickoff.md');
const contextPath = resolve('..', 'docs', 'gcsc-active-context.md');
const backlogPath = resolve('..', 'docs', 'smartcontractor-backlog.md');

function fail(message) {
  console.error(`Public beta week-two kickoff validation failed: ${message}`);
  process.exit(1);
}
...
assertIncludes(context, 'public beta week-two kickoff', contextPath);
assertIncludes(context, 'check:public-beta-week-two-kickoff', contextPath);
assertIncludes(backlog, 'Public beta week-two kickoff', backlogPath);
assertIncludes(backlog, 'check:public-beta-week-two-kickoff', backlogPath);
...
if (/sk_live_[a-z0-9]|-----BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY-----|xox[baprs]-[0-9]|service_role\s*[:=]|postgresql:\/\/|password\s*[:=]|eyJ[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}/i.test(doc)) {
  fail('Week-two kickoff must not contain real secret-looking values');
}

console.log(JSON.stringify({
  status: 'passed',
  week_two_kickoff: docPath,
