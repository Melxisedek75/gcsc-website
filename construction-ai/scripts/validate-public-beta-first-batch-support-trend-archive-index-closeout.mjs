...
const closeoutPath = resolve('..', 'docs', 'smartcontractor-public-beta-first-batch-support-trend-archive-index-closeout.md');
...
function fail(message) {
  console.error(`Public beta first batch support trend archive index closeout validation failed: ${message}`);
  process.exit(1);
}
...
for (const required of [
  'redacted support trend archive index closeout records',
  'support trend archive index',
  'internal archive index closeout metadata',
  'tester-code-only',
  'safe issue IDs',
  'safe request IDs',
  'queue item IDs',
...
const scriptName = 'check:public-beta-first-batch-support-trend-archive-index-closeout';

assertIncludes(context, 'Public beta first batch support trend archive index closeout', contextPath);
assertIncludes(context, scriptName, contextPath);
assertIncludes(context, 'Backlog count at latest audit', contextPath);
assertIncludes(backlog, 'Public beta first batch support trend archive index closeout', backlogPath);
assertIncludes(backlog, scriptName, backlogPath);
assertIncludes(audit, 'Public beta first batch support trend archive index closeout', auditPath);
...
if (/https?:\/\/[^\s)`>"]+|sk_live_[a-z0-9]|-----BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY-----|xox[baprs]-[0-9]|service_role\s*[:=]|postgresql:\/\/|password\s*[:=]|eyJ[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}/i.test(closeout)) {
  fail('Public beta first batch support trend archive index closeout must not contain real URLs or secret-looking values');
}

console.log(JSON.stringify({
  status: 'passed',
  public_beta_first_batch_support_trend_archive_index_closeout: closeoutPath,
  trend_archive_index_closeout_checked: true,
