...
function fail(message) {
  console.error(`Public beta first batch support trend internal action archive validation failed: ${message}`);
  process.exit(1);
}
...
for (const section of [
  ...
  'Required Checks',
  'Acceptance Check',
]) assertIncludes(archive, section, archivePath);

for (const required of [
  'redacted support trend internal action archives',
  'support trend internal action closeout',
  'internal action archive metadata',
  'tester-code-only',
  'safe issue IDs',
  'safe request IDs',
  'queue item IDs',
  ...
  'npm run check',
]) assertIncludes(archive, required, archivePath);
...
const scriptName = 'check:public-beta-first-batch-support-trend-internal-action-archive';

assertIncludes(context, 'Public beta first batch support trend internal action archive', contextPath);
assertIncludes(context, scriptName, contextPath);
assertIncludes(context, 'Backlog count at latest audit', contextPath);
assertIncludes(backlog, 'Public beta first batch support trend internal action archive', backlogPath);
assertIncludes(backlog, scriptName, backlogPath);
assertIncludes(audit, 'Public beta first batch support trend internal action archive', auditPath);
assertIncludes(packageJson, `"${scriptName}"`, packagePath);
assertIncludes(runner, `"${scriptName}"`, runnerPath);

if (/https?:\/\/[^\s)`>"]+|sk_live_[a-z0-9]|-----BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY-----|xox[baprs]-[0-9]|service_role\s*[:=]|postgresql:\/\/|password\s*[:=]|eyJ[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}/i.test(archive)) {
  fail('Public beta first batch support trend internal action archive must not contain real URLs or secret-looking values');
}

console.log(JSON.stringify({
  status: 'passed',
  public_beta_first_batch_support_trend_internal_action_archive: archivePath,
  trend_internal_action_archive_checked: true,
