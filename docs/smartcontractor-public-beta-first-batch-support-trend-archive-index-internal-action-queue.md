...
This archive index internal action queue does not approve:
...
Record only redacted internal archive index action queue metadata in this format:
...
Any archive index action queue item that requires Codex to reply to testers, change Supabase redirects, publish publicly, make legal/provider commitments, or handle real money must be marked `BLOCKED_FOR_EXTERNAL_ACTION`.
...
This archive index internal action queue is accepted only when local docs and validators prove that first-batch support trend archive index action queue records are redacted, tester-code-only, issue/request-id-aware, queue-item-aware, closeout-aware, archive-item-aware, trend-category-aware, founder-decision-option-aware, action-owned, support-owned, rollback-aware, no-real-money-confirmed, founder-review-ready, internal-action-queue-ready, and blocked from Codex replies to testers, raw URL storage/sharing, private tester data, external account changes, deploy changes, Supabase redirects, live Supabase writes, provider setup, real-money actions, legal/provider commitments, public launch, app stores, or destructive actions.
