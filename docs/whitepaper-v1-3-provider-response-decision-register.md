...
Record future decision states that come from provider response intake, evidence, summary, routing, and action queue records. Decision evidence rows belong in `docs/whitepaper-v1-3-provider-response-decision-evidence-template.md` after a decision row exists. Decision evidence intake belongs in `docs/whitepaper-v1-3-provider-response-decision-evidence-intake.md` before any decision evidence row is treated as usable local metadata.
