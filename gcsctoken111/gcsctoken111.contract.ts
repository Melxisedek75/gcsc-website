...
class gcsctoken111 extends Contract {
    ...
    transfer(from: Name, to: Name, quantity: Asset, memo: string): void {
        // Incoming transfer notifications from OTHER token contracts (e.g.
        // eosio.token sending XPR to this account) must not run this token's
        // transfer logic — the XPR symbol is absent from our stat table, the
        // "token does not exist" assert fires, and the WHOLE payment reverts.
        // Only execute when this contract is called directly.
